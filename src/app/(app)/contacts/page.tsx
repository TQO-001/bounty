...
export default function ContactsPage() {
  ...
  async function add(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setContacts(p => [c, ...p])
      setShowForm(false)
      setForm({ name:"", email:"", phone:"", company:"", role:"", linkedin_url:"", notes:"" })
    }
    setSaving(false)
  }
    ...
    <div className="mx-auto px-8 py-10" style={{ maxWidth: "760px" }}>
        ...
        <div className="clay p-6 mb-6" style={{ background: "var(--surface)" }}>
          ...
          <form onSubmit={add} className="grid grid-cols-2 gap-4">
            {([
              ...
              ["LinkedIn URL","linkedin_url","url",false],
            ] as [string,string,string,boolean][]).map(([l,k,t,req]) => (
              <div key={k}>
      ...
      {!loading && contacts.length > 0 && (
        <div className="space-y-3">
          {contacts.map(c => (
            <div key={c.id}
              className="clay-sm flex items-center gap-4 px-5 py-4"
