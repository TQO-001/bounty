...
export default function DocumentsPage() {
  ...
  const grouped = Object.entries(TYPE_LABELS).map(([type, label]) => ({
    type, label, docs: docs.filter(d => d.doc_type === type)
    ...
    <div className="page-pad p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--muted-2)" }}>
            {docs.length} document{docs.length !== 1 ? "s" : ""} — CVs, cover letters, IDs, links
          </p>
        </div>
        ...
        <div className="rounded-xl p-6 mb-6" style={{ background: "var(--surface)", border: "1px solid var(--amber-border)" }}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <input value={uploadForm.notes} onChange={e => setUploadForm(p => ({ ...p, notes: e.target.value }))} placeholder="e.g. v3, tailored for tech roles"
        ...
        <form onSubmit={addLink} className="rounded-xl p-6 mb-6" style={{ background: "var(--surface)", border: "1px solid var(--amber-border)" }}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input value={linkForm.name} onChange={e => setLinkForm(p => ({ ...p, name: e.target.value }))} required placeholder="My Portfolio" className="w-full px-3 py-2 rounded-lg text-sm focus:outline-none" style={{ background: "var(--input-bg)", border: "1px solid var(--border-2)", color: "var(--text)" }} />
            </div>
            <div>
              <label className="block text-xs mb-1.5" style={{ color: "var(--muted)" }}>URL *</label>
              <input value={linkForm.url} onChange={e => setLinkForm(p => ({ ...p, url: e.target.value }))} required type="url" placeholder="https://..." className="w-full px-3 py-2 rounded-lg text-sm focus:outline-none" style={{ background: "var(--input-bg)", border: "1px solid var(--border-2)", color: "var(--text)" }} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs mb-1.5" style={{ color: "var(--muted)" }}>Type</label>
              <select value={linkForm.doc_type} onChange={e => setLinkForm(p => ({ ...p, doc_type: e.target.value }))} className="w-full px-3 py-2 rounded-lg text-sm focus:outline-none" style={{ background: "var(--surface-3)", border: "1px solid var(--border-2)", color: "var(--text)" }}>
                {Object.entries(TYPE_LABELS).map(([v,l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs mb-1.5" style={{ color: "var(--muted)" }}>Notes</label>
              <input value={linkForm.notes} onChange={e => setLinkForm(p => ({ ...p, notes: e.target.value }))} placeholder="Optional" className="w-full px-3 py-2 rounded-lg text-sm focus:outline-none" style={{ background: "var(--input-bg)", border: "1px solid var(--border-2)", color: "var(--text)" }} />
            </div>
          </div>
      ...
      {!loading && docs.length === 0 && !showUpload && !showLink && (
        <div className="rounded-xl p-16 text-center" style={{ background: "var(--surface)", border: "1px dashed var(--border-2)" }}>
          ...
          <p className="text-sm mb-6" style={{ color: "var(--muted-2)" }}>Upload your CV, ID, cover letters, or add links to your portfolio.</p>
            ...
            {g.docs.map(doc => (
              <div key={doc.id} className="flex items-center gap-4 px-5 py-4 rounded-xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                ...
                <div className="flex items-center gap-1">
                  {doc.is_link && doc.url && (
                    <a href={doc.url} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style={{ color: "var(--muted-2)" }} title="Open link">
                      <ExternalLink size={14} />
                    </a>
                  )}
                  {!doc.is_link && (
                    <a href={"/api/documents/" + doc.id + "/file"} download className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style={{ color: "var(--muted-2)" }} title="Download">
                      <Download size={14} />
                    </a>
                  )}
                  <button onClick={() => del(doc.id)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style={{ color: "var(--muted)" }} title="Delete">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
