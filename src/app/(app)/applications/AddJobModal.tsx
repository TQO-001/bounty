        ...
        <div className="flex items-start gap-4 p-6 pb-0">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1">
              <div className="w-5 h-5 rounded flex items-center justify-center text-xs font-bold flex-shrink-0" style={{ background: form.company_name ? "var(--amber-dim)" : "var(--surface-3)", color: "var(--amber)", border: "1px solid var(--amber-border)" }}>
          ...
          {tab === "details" && (
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Sel label="Work type" value={form.work_type} onChange={v=>set("work_type",v)} options={["","onsite","remote","hybrid"]} labels={{"":"Any","onsite":"On-site","remote":"Remote","hybrid":"Hybrid"}} />
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 px-3 py-2 rounded-t-lg" style={{ background: "var(--surface-3)", border: "1px solid var(--border-2)", borderBottom: "none" }}>
                  {["B","I","U"].map(f => <button key={f} type="button" className="text-xs font-bold w-6 h-6 rounded" style={{ color: "var(--muted-2)" }}>{f}</button>)}
                </div>
                <textarea value={form.job_description} onChange={e=>set("job_description",e.target.value)} rows={4}
                  placeholder="Paste the job description here…"
                  className="w-full px-3 py-2.5 rounded-b-lg text-sm focus:outline-none resize-none"
                  style={{ background: "var(--input-bg)", border: "1px solid var(--border-2)", borderTop: "none", color: "var(--text)" }} />
              </div>
            </>
          )}
