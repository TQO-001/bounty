...
export function ActivityLog({ appId, initialEvents }: { appId: string; initialEvents: Event[] }) {
  ...
  return (
    <div className="rounded-xl p-5" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
      ...
      {showForm && (
        <form onSubmit={addEvent} className="rounded-lg p-4 mb-4 space-y-3" style={{ background: "var(--surface-2)", border: "1px solid var(--border-2)" }}>
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={saving} className="text-xs px-3 py-1.5 rounded-lg font-semibold text-black disabled:opacity-50" style={{ background: "var(--amber)" }}>
              {saving ? "Saving…" : "Save"}
            </button>
        ...
        <div className="space-y-0">
          {events.map((ev, i) => (
            <div key={ev.id} className="flex gap-4 relative">
              ...
              <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-1 z-10" style={{ background: "var(--surface-3)", border: "1px solid var(--border-2)", color: "var(--muted-2)" }}>
