...
import { LayoutGrid, List, Table2, Plus, Search, Filter, Download, Trash2, SlidersHorizontal } from "lucide-react"
...
export function ApplicationsView({ allApps, filtered, counts, filterStatus, searchQuery, initialView }: {
  ...
  const exportIds = selected.size > 0 ? [...selected] : apps.map(a=>a.id)
  async function exportCSV() {
    setExporting(true)
    const res = await fetch("/api/applications/export?format=csv",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ids:exportIds})})
    const blob = await res.blob(); const url = URL.createObjectURL(blob)
    const a = document.createElement("a"); a.href=url; a.download="bounty.csv"; a.click(); URL.revokeObjectURL(url)
    setExporting(false)
  }
    ...
    <div className="flex flex-col" style={{ height: "calc(100vh - 56px)" }}>
      ...
      <div className="flex items-center gap-2 px-5 py-2.5 flex-shrink-0" style={{ borderBottom: "1px solid var(--border)" }}>
        ...
        <div className="flex items-center gap-1 mr-1">
          {([["kanban","Board",LayoutGrid],["list","List",List],["table","Table",Table2]] as const).map(([v,label,Icon]) => (
            <button key={v} onClick={() => setView(v)}
              ...
              style={{ background: view===v?"var(--amber-dim)":"transparent", color: view===v?"var(--amber)":"var(--muted-2)", border: view===v?"1px solid var(--amber-border)":"1px solid transparent" }}>
              <Icon size={13} />{label}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-1 overflow-x-auto hide-mobile" style={{ flex: 1 }}>
          <button onClick={() => pushFilter({})}
            className={`text-xs px-2.5 py-1 rounded-lg font-medium whitespace-nowrap transition-all ${!filterStatus ? "text-amber-400" : ""}`}
          ...
          {PIPELINE_COLS.filter(s => counts[s] > 0).map(s => (
            <button key={s} onClick={() => { setSelected(new Set()); pushFilter({status:s}) }}
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          <button onClick={exportCSV} disabled={exporting} className="view-btn hide-mobile" title="Export CSV"><Download size={13}/></button>
          {selected.size>0 && (
            <button onClick={bulkDelete} disabled={bulkLoading} className="text-xs px-2.5 py-1.5 rounded-lg" style={{ border:"1px solid rgba(239,68,68,0.3)", color:"#f87171" }}>
      ...
      {/* Filter bar */}
      {showFilters && (
        <div className="flex items-center gap-3 px-5 py-2 flex-shrink-0 flex-wrap" style={{ background:"var(--surface-2)", borderBottom:"1px solid var(--border)" }}>
          ...
          {[
            ...
            <div key={f.key} className="flex items-center gap-1.5">
              ...
              <select value={(filters as Record<string,string>)[f.key]} onChange={e=>setFilters(p=>({...p,[f.key]:e.target.value}))}
