...
export function ListView({ apps, selected, setSelected }: { apps: Application[]; selected: Set<string>; setSelected: (s: Set<string>) => void }) {
  const router = useRouter()
  if (!apps.length) return <div className="flex items-center justify-center h-64"><p className="text-sm" style={{ color: "var(--muted)" }}>No applications found.</p></div>
  return (
      ...
      {apps.map(app => {
        ...
        return (
          <div key={app.id}
            ...
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-base font-bold flex-shrink-0" style={{ background: "var(--amber-dim)", color: "var(--amber)", border: "1px solid var(--amber-border)" }}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                ...
                <span className="text-xs font-medium px-2.5 py-1 rounded-md flex-shrink-0" style={{ background: c.bg, color: c.color }}>{STATUS_LABELS[app.status]}</span>
              </div>
