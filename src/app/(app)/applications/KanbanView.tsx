...
const COL_ACCENT: Record<ApplicationStatus, string> = {
  wishlist:"#a1a1aa", applied:"#60a5fa", phone_screen:"#fbbf24",
  interview:"#a78bfa", offer:"#34d399", rejected:"#f87171",
  withdrawn:"#fb923c", ghosted:"#94a3b8",
}
...
const WORK_TAG_COLORS: Record<string,{bg:string;color:string}> = {
  remote:  { bg:"rgba(59,130,246,0.12)",  color:"#60a5fa" },
  onsite:  { bg:"rgba(16,185,129,0.12)",  color:"#34d399" },
  hybrid:  { bg:"rgba(167,139,250,0.12)", color:"#a78bfa" },
}
const STATUS_TAG: {bg:string;color:string} = { bg:"rgba(245,158,11,0.12)", color:"#f59e0b" }
...
export function KanbanView({ apps, allApps }: { apps: Application[]; allApps: Application[] }) {
  ...
  async function handleDrop(targetCol: ApplicationStatus) {
    ...
    await fetch("/api/applications/" + id, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: targetCol }) })
    router.refresh()
  }
    ...
    <>
      <div className="kanban-board">
        {VISIBLE.map(col => {
          ...
          return (
            <div key={col} className={`kanban-col ${overCol === col ? "drag-over" : ""}`}
              ...
              <div className="kanban-col-header">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: "var(--surface-3)", color: "var(--muted-2)" }}>{colApps.length}</span>
                </div>
                <div className="flex items-center gap-1">
                  <button className="w-6 h-6 flex items-center justify-center rounded-lg transition-colors" style={{ color: "var(--muted)" }}>
              ...
              <div className="kanban-col-body">
                ...
                {colApps.length === 0 && (
                  <div className="py-8 flex flex-col items-center gap-2">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "var(--surface-2)", border: "1px dashed var(--border-2)" }}>
      ...
      {addingToCol && (
        <AddJobModal defaultStatus={addingToCol} onClose={() => setAddingToCol(null)} />
      )}
    </>
  )
}
...
function KanbanCard({ app, isDragging, onDragStart, onDragEnd, onClick }: {
  app: Application; isDragging: boolean; onDragStart: () => void; onDragEnd: () => void; onClick: () => void
}) {
    ...
    <div draggable onDragStart={onDragStart} onDragEnd={onDragEnd} onClick={onClick}
      ...
      {app.salary_max && (
        <p className="text-xs mb-2.5" style={{ color: "var(--muted-2)" }}>
          ...
          {app.salary_min ? ` – US$${Number(app.salary_max).toLocaleString()}` : ""}
        </p>
      )}

      {/* Notes preview */}
      {app.notes && !app.salary_max && (
      ...
      <div className="flex items-center gap-1.5 flex-wrap mt-1">
        ...
        {app.location && (
          <span className="job-tag flex items-center gap-0.5" style={{ background: "rgba(128,128,128,0.1)", color: "var(--muted-2)" }}>
