...
const VISIBLE: ApplicationStatus[] = [
...
export function KanbanView({
  ...
  // FIXED: sync items state when the apps prop changes (e.g. after router.refresh())
  ...
  return (
    <>
      <div className="kanban-board">
        ...
        {/* Add column button */}
        <button
          onClick={() => setAddingToCol("wishlist")}
      ...
      {addingToCol && (
        <AddJobModal
          ...
          onClose={() => { setAddingToCol(null); router.refresh() }}
        />
      )}
    </>
  )
...
function KanbanCard({
  ...
  return (
    ...
    >
      ...
      {/* Job title */}
      <p className="text-sm font-semibold mb-1 leading-snug" style={{ color: "var(--text)" }}>
        {app.job_title}
      </p>

      {/* FIXED: salary range now correctly shows min–max instead of max–max */}
      ...
      {/* Notes preview (only when no salary) */}
      {app.notes && !app.salary_min && !app.salary_max && (
        <p className="text-xs mb-2.5 line-clamp-2" style={{ color: "var(--muted)" }}>
          {app.notes}
        </p>
      )}
      ...
      <div className="flex items-center gap-1.5 flex-wrap mt-1">
        {app.work_type && workTagColor && (
          <span
            className="job-tag"
            style={{ background: workTagColor.bg, color: workTagColor.color }}
          >
        ...
        {app.location && (
          <span
            className="job-tag flex items-center gap-0.5"
            style={{ background: "rgba(128,128,128,0.1)", color: "var(--muted-2)" }}
          >
            <MapPin size={8} />
            {app.location}
          </span>
      ...
      >
        <span className="text-xs" style={{ color: "var(--muted)" }}>
          {app.application_date
            ? new Date(app.application_date).toLocaleDateString("en", {
                month: "short",
                day: "numeric",
              })
            : "No date"}
