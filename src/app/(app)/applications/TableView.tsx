...
export function TableView({ apps, selected, setSelected }: {
          ...
          <tr key={app.id} className={selected.has(app.id) ? "row-selected" : ""} onClick={() => router.push("/applications/" + app.id)}>
            ...
            <td>
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded flex items-center justify-center text-xs font-bold flex-shrink-0" style={{ background: "var(--amber-dim)", color: "var(--amber)", border: "1px solid var(--amber-border)" }}>
...
function StatusDropdown({ appId, status }: { appId: string; status: ApplicationStatus }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 rounded-xl shadow-2xl z-50 py-1 min-w-[145px]" style={{ background: "var(--surface-3)", border: "1px solid var(--border-2)" }}>
          {STATUS_ORDER.map(s => (
            <button key={s} onClick={() => update(s)} className="w-full text-left px-3 py-2 text-xs"
              ...
              onMouseEnter={e => { if (s !== current) e.currentTarget.style.background = "rgba(128,128,128,0.06)" }}
