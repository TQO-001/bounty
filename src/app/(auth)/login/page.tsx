...
export default function LoginPage() {
  ...
  return (
    <div className="rounded-2xl p-8" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
      <h1 className="text-lg font-bold mb-6" style={{ color: "var(--text)" }}>Welcome back</h1>
      ...
      <form onSubmit={submit} className="space-y-4">
        {[["Email","email","text"],["Password","password","password"]].map(([l,k,t]) => (
        ...
        <button type="submit" disabled={loading} className="w-full py-2.5 rounded-lg text-sm font-semibold text-black disabled:opacity-60" style={{ background: "var(--amber)" }}>{loading ? "Signing in…" : "Sign in"}</button>
      </form>
      <p className="text-center text-sm mt-5" style={{ color: "var(--muted)" }}>No account? <Link href="/register" style={{ color: "var(--amber)" }}>Sign up</Link></p>
    </div>
  )
}
