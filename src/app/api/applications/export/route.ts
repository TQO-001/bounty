...
export async function POST(req: NextRequest) {
  try {
    ...
    if (format === "csv") {
      ...
      return new NextResponse(csv, { headers: { "Content-Type": "text/csv", "Content-Disposition": "attachment; filename=\"bounty-export.csv\"" } })
    }
    ...
    const rows = apps.map((a: Record<string,unknown>) => `
      <tr>
        ...
        <td>${a.location ?? ""}</td>
        <td>${a.salary_max ? "US$" + Number(a.salary_max).toLocaleString() : ""}</td>
        <td>${a.application_date ? new Date(String(a.application_date)).toLocaleDateString() : ""}</td>
        <td>${a.priority ?? ""}</td>
      </tr>`).join("")
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Bounty Export</title>
    <style>body{font-family:sans-serif;font-size:12px;padding:32px;}h1{font-size:18px;margin-bottom:4px;}p{color:#888;margin-bottom:24px;}table{width:100%;border-collapse:collapse;}th{background:#f5f5f0;text-align:left;padding:8px 10px;font-size:11px;text-transform:uppercase;letter-spacing:.05em;}td{padding:8px 10px;border-bottom:1px solid #eee;}@media print{body{padding:0;}}</style>
    </head><body><h1>Bounty — Job Applications</h1><p>Exported ${new Date().toLocaleDateString()} · ${apps.length} application${apps.length !== 1 ? "s" : ""}</p>
    <table><thead><tr><th>Role</th><th>Company</th><th>Status</th><th>Location</th><th>Max Salary</th><th>Applied</th><th>Priority</th></tr></thead>
