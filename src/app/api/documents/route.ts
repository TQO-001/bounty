...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes("application/json")) {
      const { name, url, doc_type, notes, is_link } = await req.json()
    ...
    const rows = await sql`
      INSERT INTO documents (user_id, name, doc_type, storage_path, file_size, notes, is_default, is_link)
      VALUES (${user.userId}, ${file.name}, ${doc_type}, ${filename}, ${file.size}, ${notes || null}, false, false)
      RETURNING *`
    return NextResponse.json(rows[0], { status: 201 })
